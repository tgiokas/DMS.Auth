...
public class UserManagementService : IUserManagementService
{
    ...
    public async Task<Result<Dtos.PagedResult<UserProfileDto>>> GetUsersAsync(UserQueryParams queryParams)
    {
        ...
        // The local auth DB (users who have actually logged in) is the source of truth for the user list,
        // NOT the full Keycloak/LDAP directory              
        ...
        else if (queryParams.Filters != null && queryParams.Filters.Any())
        {
            ...
            if (filtersMap.TryGetValue("id", out var fids) && !string.IsNullOrWhiteSpace(fids))
            {
                var userIds = new HashSet<string>(fids.Split(",", StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries), StringComparer.OrdinalIgnoreCase);
                userProfiles = userProfiles.Where(u => userIds.Contains(u.Id)).ToList();
            }
            if (filtersMap.TryGetValue("roles.id", out var froleId) && !string.IsNullOrWhiteSpace(froleId))
            {
                await EnrichRolesAsync(userProfiles);
                rolesLoaded = true;

                userProfiles = userProfiles.Where(x => x.Roles.Any(r => r.Id.Equals(froleId, StringComparison.OrdinalIgnoreCase))).ToList();

                if (!includeRoles)
                {
                    // don't return role data
                    userProfiles.ForEach(u => u.Roles = new List<RoleProfileDto>());
                }
            }
    ...
    public async Task<Result<List<UserProfileWithDeletedStatusDto>>> GetUsersByIdsAsync(List<IdDto> userIds)
    {
        var userProfiles = new List<UserProfileWithDeletedStatusDto>();
        ...
        foreach (var userId in userIds)
        {
            ...
            userProfiles.Add(MapToUserProfile<UserProfileWithDeletedStatusDto>(keycloakUser, localUser, roles));
        }

        if (userProfiles.Count == 0)
        {
            return _errors.Fail<List<UserProfileWithDeletedStatusDto>>(ErrorCodes.AUTH.UsersNotFound);
        }
        ...
        return Result<List<UserProfileWithDeletedStatusDto>>.Ok(userProfiles, message);
    }
    ...
    private static UserProfileDto MapToUserProfile(KeycloakUser keycloakUser, User? localUser, List<RoleProfileDto>? roles = null)
    { 
        return MapToUserProfile<UserProfileDto>(keycloakUser, localUser, roles);
    }

    private static TProfile MapToUserProfile<TProfile>(KeycloakUser keycloakUser, User? localUser, List<RoleProfileDto>? roles = null)
        where TProfile : UserProfileDto, new()
    {
